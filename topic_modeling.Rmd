...
speech_top_15_terms <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 1) %>%
  head(10) %>%
  arrange(-beta)

topic2_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 2) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic2_top)

topic3_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 3) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic3_top)

topic4_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 4) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic4_top)

topic5_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 5) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic5_top)

topic6_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 6) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic6_top)

topic7_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 7) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic7_top)

topic8_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 8) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic8_top)

topic9_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 9) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic9_top)

topic10_top <- speech_lda_tidy %>%
  group_by(topic) %>%
  filter(topic == 10) %>%
  head(10) %>%
  arrange(-beta)
speech_top_15_terms <- rbind(speech_top_15_terms, topic10_top)
...
speech_top_terms_graph <- speech_top_15_terms %>%
...
tm_time_doc <- corpus_relevant_modified %>%
  unnest_tokens(word, cleaned_text) %>%
  mutate(term = wordStem(word)) %>%
  anti_join(stop_words) %>%
  select(id, slug, term)

document_topic_richness <- tm_time_doc %>%
  full_join(speech_top_50_terms_wider, by = "term") %>%
  group_by(id) %>%
  summarize(t1_sum = sum(t1, na.rm = TRUE), t2_sum = sum(t2, na.rm = TRUE), t3_sum = sum(t3, na.rm = TRUE), 
            t4_sum = sum(t4, na.rm = TRUE), t5_sum = sum(t5, na.rm = TRUE), t6_sum = sum(t6, na.rm = TRUE), 
            t7_sum = sum(t7, na.rm = TRUE), t8_sum = sum(t8, na.rm = TRUE), t9_sum = sum(t9, na.rm = TRUE),
            t10_sum = sum(t10, na.rm = TRUE), slug = min(slug)) %>%
  select(id, slug, t1_sum, t2_sum, t3_sum, t4_sum, t5_sum, t6_sum, t7_sum, t8_sum, t9_sum, t10_sum)
