...
dictionary_original <- read_csv("intent_dictionary.csv") %>%
  mutate(word = str_replace_all(tolower(value), "[[:punct:]]", " ")) %>%
  select(word)

dictionary_stemmed <- read_csv("stemmed_intent_dictionary.csv")

dictionary_occurrences <- read_csv("final_intent_dictionary_occurences_all.csv")
```

Here is the raw dictionary of INTENT terms we looked for; for reference, terms were stemmed and lemmatized so that variants of the same term or phrase could be considered as one. Additionally, I removed stop words from all terms (common, non-substantive words) for ease of analysis with the full corpus.
...
dictionary_occurrences %>%
  select(slug, id) %>%
  group_by(id) %>%
  summarise(count = n(), slug = max(slug)) %>%
  select(slug, id, count) %>%
  arrange(-count) %>%
  head(50) %>%
  gt() %>%
  tab_header(title = "Top Cases - By Raw Counts") %>%
  cols_label(id = "Case ID", slug = "Case", count = "Total Uses of Dictionary Terms")
```
...
case_richness <- dictionary_cases %>%
  ...
  arrange(-richness) %>%
  select(slug, id, richness) %>%
  head(50) %>%
  gt() %>%
  tab_header(title = "Top Cases - By Dictionary Richness") %>%
  cols_label(slug = "Case", id = "Case ID", richness = "Percent of Dictionary Terms in Case")
...
case_richness_overall <- dictionary_cases %>%
  right_join(corpus_relevant, by = "id") %>%
  mutate(slug = slug.x) %>%
  mutate(richness = 100*(count/doc_length_post_clean)) %>%
