...
search_for_single_word(corpus_40s, "diversity")
...
search_for_single_word_graph <- function(corpus_for_search, input_word){
  ...
  num_occurences_data <- left_join(num_occurences_tibble, yearly_totals, by = "year") %>%
  ...
  num_occurences_graph <- num_occurences_data %>%
    ...
    geom_line() + 
    scale_x_continuous(breaks = round(seq(min(num_occurences_data$year), max(num_occurences_data$year), by = 1),1)) +
    scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +
    labs(title = paste("Occurences of Term: '", stem_to_look_for, "' in Given Corpus", sep = ""),
...
search_for_single_word_graph(corpus_40s, "segregation")
...
## MULTIPLE WORDS: NEED TO FIX (12/4)

search_for_multiple_words <- function(corpus_for_search, words){
  
  num_cases = nrow(corpus_for_search)
  
  tokens <- corpus_to_search %>%
    unnest_tokens(word, clean_text) %>%
    anti_join(get_stopwords()) %>%
    select(word)
  ...
  raw_words <- tibble(raw_word = character())
  
  for(i in length(words)){
    raw_words <- rbind(raw_words, words[i])
  }
  ...
  occurences <- right_join(token_stems, stem_words, by = "word")
...
for(i in length(test_words)){
  raw_words <- raw_words %>%
      add_row(raw_word = test_words[i])
}
